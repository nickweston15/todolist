...
module.exports = class PgPersistence {
  ...
  // Toggle a todo between the done and not done state. Returns a promise
  // that resolves to 'true' on success, 'false' if the todo list or todo doesn't
  ...
  // Delete the specified todo from the specified todo list. Returns a promise
  // that resolves to 'true' on success, 'false' if the todo list or todo
  ...
  async deleteTodo(todoListId, todoId) {
    ...
    let result = dbQuery(DELETE_TODO, todoListId, todoId, this.username);
    return result.rowCount > 0;
  }
  ...
  async deleteTodoList(todoListId) {
    ...
    let result = dbQuery(DELETE_TODOLIST, todoListId, this.username);
    return result.rowCount > 0;
  }

  // Mark all todos on the todo list as done. Returns a promise that resolves
  // to `true` on success, `false` if the todo list doesn't exist. The todo
